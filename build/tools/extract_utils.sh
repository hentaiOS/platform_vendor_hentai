...
function setup_vendor_deps() {
    ...
    export CLANG_BINUTILS="$HENTAI_ROOT"/prebuilts/clang/host/${HOST}-x86/llvm-binutils-stable
        ...
        if [ "$CLASS" = "SHARED_LIBRARIES" ]; then
            ...
            if [ "$EXTRA" = "both" ]; then
                ...
                if [ -z "$DISABLE_CHECKELF" ]; then
                    printf '\t\t\tshared_libs: [%s],\n' "$(basename -s .so $(${OBJDUMP} -x "$HENTAI_ROOT"/"$OUTDIR"/"$SRC"/lib/"$FILE" 2>/dev/null |grep NEEDED) 2>/dev/null |grep -v ^NEEDED$ |sed 's/-3.9.1//g' |sed 's/\(.*\)/"\1",/g' |tr '\n' ' ')"
                fi
                printf '\t\t},\n'
                printf '\t\tandroid_arm64: {\n'
                printf '\t\t\tsrcs: ["%s/lib64/%s"],\n' "$SRC" "$FILE"
                if [ -z "$DISABLE_CHECKELF" ]; then
                    printf '\t\t\tshared_libs: [%s],\n' "$(basename -s .so $(${OBJDUMP} -x "$HENTAI_ROOT"/"$OUTDIR"/"$SRC"/lib64/"$FILE" 2>/dev/null |grep NEEDED) 2>/dev/null |grep -v ^NEEDED$ |sed 's/-3.9.1//g' |sed 's/\(.*\)/"\1",/g' |tr '\n' ' ')"
                fi
                printf '\t\t},\n'
            elif [ "$EXTRA" = "64" ]; then
                printf '\t\tandroid_arm64: {\n'
            ...
            if ! objdump -a "$HENTAI_ROOT"/"$OUTDIR"/"$SRC"/bin/"$FILE" 2>/dev/null |grep -c 'file format elf' > /dev/null; then
            ...
            if [ "$EXTENSION" != "sh" ]; then
                printf '\ttarget: {\n'
                if objdump -a "$HENTAI_ROOT"/"$OUTDIR"/"$SRC"/bin/"$FILE" |grep -c 'file format elf64' > /dev/null; then
                    printf '\t\tandroid_arm64: {\n'
                else
                    printf '\t\tandroid_arm: {\n'
                fi
                printf '\t\t\tsrcs: ["%s/bin/%s"],\n' "$SRC" "$FILE"
                if [ -z "$DISABLE_CHECKELF" ]; then
                    printf '\t\t\tshared_libs: [%s],\n' "$(basename -s .so $(${OBJDUMP} -x "$HENTAI_ROOT"/"$OUTDIR"/"$SRC"/bin/"$FILE" 2>/dev/null |grep NEEDED) 2>/dev/null |grep -v ^NEEDED$ |sed 's/-3.9.1//g' |sed 's/\(.*\)/"\1",/g' |tr '\n' ' ')"
                fi
                printf '\t\t},\n'
                printf '\t},\n'
                if objdump -a "$HENTAI_ROOT"/"$OUTDIR"/"$SRC"/bin/"$FILE" |grep -c 'file format elf64' > /dev/null; then
...
function write_rro_package() {
    ...
    local RROBP="$HENTAI_ROOT"/"$OUTDIR"/rro_overlays/"$PKGNAME"/Android.bp
    local RROMANIFEST="$HENTAI_ROOT"/"$OUTDIR"/rro_overlays/"$PKGNAME"/AndroidManifest.xml
...
function parse_file_list() {
    ...
    done < <(grep -Ev '(^#|^[[:space:]]*$)' "$LIST" | LC_ALL=C sort | uniq)
}
