...
SIGNED_TARGET_FILES_PACKAGE := $(PRODUCT_OUT)/$(TARGET_DEVICE)-target_files-$(BUILD_ID_LC)-$(BUILD_ID_DATE).zip
...
.PHONY: prod-build
prod-build: $(PROD_OTA_PACKAGE_TARGET)
...
$(PROD_UPDATE_PACKAGE_TARGET): $(SIGNED_TARGET_FILES_PACKAGE) \
	...
	@echo "helluva updatepackage: $@"
	    $(IMG_FROM_TARGET_FILES) \
	        --additional IMAGES/VerifiedBootParams.textproto:VerifiedBootParams.textproto \
	        $(SIGNED_TARGET_FILES_PACKAGE) $@

.PHONY: prod-updatepackage
prod-updatepackage: $(PROD_UPDATE_PACKAGE_TARGET)
...
$(INCREMENTAL_OTA_PACKAGE_TARGET): $(SIGNED_TARGET_FILES_PACKAGE) \
		$(INTERNAL_OTATOOLS_FILES) $(OTA_FROM_TARGET_FILES)
	@echo "helluva incremental build: $@"
	    $(OTA_FROM_TARGET_FILES) --verbose \
	    -p $(OUT_DIR)/host/linux-x86 \
	    -k $(KEY_CERT_PAIR) \
...
.PHONY: prod-incremental
prod-incremental: $(INCREMENTAL_OTA_PACKAGE_TARGET)
